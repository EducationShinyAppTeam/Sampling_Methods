...
dataf.sub$Median.Household.Income <- gsub(",",
                                          "",
                                          dataf.sub$Median.Household.Income )
...
missingCounty <- data.frame("Shannon", "South Dakota", "46113", "7,341", "51.9",
                            "27,804", "2161", "2251", "3421", "1080", "24", "25",
                            "38", "12", "14,291", "11.7", "12.08", "10.3")
...
final.data$Square_Kilometers <- round(area(counties.ordered)/1000000, 
                                      digits = 2)

final.data$Population <- gsub(",",
                              "", 
                              final.data$Population)

final.data$Population <- as.numeric(final.data$Population)

final.data$Median.Household.Income <- gsub(",",
                                           "", 
                                           final.data$Median.Household.Income)
...
counties.UR2 <- counties.ordered[counties.ordered$Pop.Per.Square.Kilometer > 9.3124 &
                                   counties.ordered$Pop.Per.Square.Kilometer <= 31.1977, ]

counties.UR3 <- counties.ordered[counties.ordered$Pop.Per.Square.Kilometer > 31.1977 & 
...
a <- c('Alexandria', 'Bristol', 'Buena Vista', 'Charlottesville', 'Chesapeake', 
       ...
       'Martinsville', 'Norton', 'Petersburg', 'Poquoson', 'Portsmouth', 
       'Radford', 'Richmond City', 'Roanoke City', 'Salem', 'Staunton', 'Waynesboro', 
...
counties.position2 <- subset(counties.position,
                             select = c(NAME, 
                                        STATE_NAME))
...
counties.position2 <- mutate(counties.position2@data,
                             ID = row_number())
...
shinyServer(function(input, output, session) {
  ...
  addPopover(session = session, 
             id = "systemTable", 
             title = "Note:",
             content = "Notice how the difference between the ID's of the counties 
                        displayed corresponds to the interval calculated above", 
             trigger = "hover",
             placement = "right")
  ...
  addPopover(session = session,
             id = "myBoxplots", 
             title = "Note:",
             content = "Notice when the strata variable is geographic region, there
                        appears to be a larger difference between strata regarding
                        unemployment rate than when the strata variable is population per square 
                        kilometer. The strata variable makes a difference in this case!",
             trigger = "hover", 
             placement = "left")
  ...
  observeEvent(input$info,{
    sendSweetAlert(
      session = session,
      title = "Instructions:",
      text = "This app explores the differences between the four most commonly 
              used sampling methods: Simple Random Sampling, Stratified Sampling, 
  ...
  output$myMap <- renderPlot({
    if (input$generate == 0)
      return( plot(counties.ordered) +
                title("US Counties"))
    
    plot(counties.ordered) + 
      title("Counties in First Sample")
    ...
    plot(counties.ordered[randomdata, ],
         col = "red", 
         add = TRUE)
  })
  ...
  output$first50Counties <- renderPrint({
    ...
    randomdataSub
    cat(paste(final.data[randomdataSub,]$NAME,
              collapse = ",  ")) 
  })
  ...
  output$samplesHistogram <- renderPlot({
    ...
    sampleMeans <- replicate(sampleCount, 
                             mean(sample(final.data$Median.Household.Income, 
                                      sampleSize)))
    ...
    title("Histogram of Median Household Income Across Samples",
          line = 3.4)
    ...
    abline(v = mean(sampleMeans),
           col = "red",
           lwd = 4,
           lty = "dotted")
    
    legend("topleft", 
           legend = c("True Population Median Household Income ($50,957.72)",
                      "Average Median Household Income Across Samples"), 
           col =
             c("blue", "red"),
           bty = "n", 
           pch = 18,
  ...
  output$myMap2 <- renderCachedPlot(
    {
      ...
      if (isolate(input$strataVar) == "Geographic Region"){
        plot(counties.ordered) +
          title("Stratified US Counties: Geographic Region")
        
        plot(counties.northEast, 
             col = "#B0DAFF", 
             add = TRUE)
        
        plot(counties.northEast[s$NE, ], 
             col = c$NE, add = TRUE)
        
        plot(counties.midWest,
             col = "#FFC795", 
             add = TRUE)
        
        plot(counties.midWest[s$MW, ], 
             col = c$MW, 
             add = TRUE)
        ...
        plot(counties.West[s$W, ],
             col = c$W, 
             add = TRUE)
        
        plot(counties.southEast, 
             col = "darkolivegreen1",
             add = TRUE)
        
        plot(counties.southEast[s$SE, ],
             col = c$SE,
             add = TRUE)
        
        plot(counties.southWest, 
             col = "gray68",
             add = TRUE)
        
        plot(counties.southWest[s$SW, ], 
             col = c$SW, 
             add = TRUE)
      ...
      if (isolate(input$strataVar) == "Pop. Per Square Kilometer"){
        plot(counties.ordered) + 
          title("Stratified US Counties: Population Per Square Kilometer")
        
        plot(counties.UR1, 
             col = "#FFE2B3", 
             add = TRUE)
        
        plot(counties.UR1[s$Low, ],
             col = c$Low,
             add = TRUE)
    ...
    cacheKeyExpr = {list(input$generate2, (input$strataVar), 
                         input$generate3,
                         (s$NE), (c$NE), (s$MW), 
                         (c$MW), (s$W), (c$W), 
                         (s$SW), (c$SW), (s$SE), 
  ...
  output$myBoxplots <- renderPlot({
    ...
    if ((input$strataVar) == "Geographic Region"){
      boxplot(counties.northEast[s$NE, ]$Unemployment.Rate, 
              counties.midWest[s$MW, ]$Unemployment.Rate,
              counties.West[s$W, ]$Unemployment.Rate, 
              counties.southEast[s$SE, ]$Unemployment.Rate,
              counties.southWest[s$SW, ]$Unemployment.Rate,
              main = "Unemployment Rate Per Strata",
              ylab = "", 
              xlab = "", 
    ...
    if ((input$strataVar) == "Pop. Per Square Kilometer"){
      boxplot(counties.UR1[s$Low, ]$Unemployment.Rate, 
              counties.UR2[s$Med, ]$Unemployment.Rate, 
              counties.UR3[s$High, ]$Unemployment.Rate, 
              ylim = c(1,
                       7.5), 
              main = "Unemployment Rate Per Strata (Sample)",
  ...
  output$strataCalc <- renderUI({
    ...
    geoTotal <- paste("Total: ",
                      round((sampleSize/3108)*366), 
                      "+", 
                      round((sampleSize/3108)*1055),
                      "+",
                      round((sampleSize/3108)*379),
                      "+",
                      round((sampleSize/3108)*245),
                      "+",
                      round((sampleSize/3108)*1063)," ≈ ",
                      strong(sampleSize))
    
    popTotal <- paste("Total: ",
                      round((sampleSize/3108)*1026), 
                      "+", 
                      round((sampleSize/3108)*1056), 
                      "+", 
                      round((sampleSize/3108)*1026),
                      " ≈ ", 
                      strong(sampleSize))
  ...
  stateSample <- reactive({
    ...
    sample(c("Alabama", "Arizona", "Arkansas", "California", "Colorado",
             ...
             "Oregon", "Pennsylvania", "Rhose Island", "South Carolina", 
  ...
  output$myMap3 <- renderPlot({
    ...
    plot(counties.ordered) + 
      title("Clustered US Counties by State")
    ...
    plot(counties.ordered[counties.ordered@data$STATE_NAME %ni% stateSample, ],
         col = c2$Cluster2,
         add = TRUE)
  })
  ...
  output$clusterLegend <- renderText({
    ...
    paste("Note: The blue, yellow, orange, and green colors shown above are 
          solely used to define state boundaries.
  ...
  output$startingCounty <- renderPrint({
    ...
    cat(paste(counties.position[start,]$NAME,
              ",",counties.position[start,]$STATE_NAME, 
              "(", randomNumber(), ")",
              collapse = ", "))
  })
  ...
  output$myMap4 <- renderPlot({
    if (input$generate6 == 0)
      return(plot(counties.position) +
               title("US Counties"))
    
    randomSystem <- isolate(randomSystem())
    plot(counties.position) + 
      title("US Counties")
    
    plot(counties.position[randomSystem,],
         col = "red", 
         add = TRUE)
    
    plot(counties.position[randomSystem[1],], 
         col = "lawngreen", 
         add = TRUE)
  ...
  output$mapLegend <- renderText({
    ...
    paste("Note: the starting county is highlighted in green (If you can't find 
          it, try generating a new starting point)")
  ...
  output$systemTable <- renderTable(
    head(counties.position2[randomSystem(),]),
    spacing = "xs", 
    width = '250px'
  )
  ...
  output$systemSummary <- renderPlot({
    ...
    randomSystem <- isolate(randomSystem())
    boxplot(counties.position$Birth.Rate, 
            counties.position[randomSystem, ]$Birth.Rate, 
            main = "Birth Rate across Counties", 
            names = c("Population", 
                      "Sample"),
            col = c("blue", 
                    "red"),
